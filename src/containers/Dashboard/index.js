...
export default class Dashboard extends Component {
  ...
  setFile = async (values) => {
    ...
    const isAddingFileFlag = validObjectWithParameterKeys(formValues, ['fileName']);
    ...
    let mdStr = '---\n';
	  ...
	  validMetaDataKeys
		  ...
		  .forEach(k => {
		  	// if key === 'image' && it is not default image get relative aws url
			  mdStr += addKeyValuePairAsString(k,
				  k === 'image' && !dataObj[k]
				  ? DEFAULT_BLOG_IMAGE_URL
				  : (k === 'image' && dataObj[k] !== DEFAULT_BLOG_IMAGE_URL ? getAbsoluteS3FileUrl(dataObj[k]) : dataObj[k]),
				  '\n'
			  );
      });
  ...
  render () {
    ...
    const validBitBucketListArray = Array.isArray(bitBucketList) && !!bitBucketList.length;
    const validBitBucketListFlag = loadingCompleteFlag && validBitBucketListArray;
						...
						
						      {
							      ...
							      <div className="content">
								      <span style={{ color: 'red' }}>
									      {
									      	loadErr ||
										      (Array.isArray(bitBucketList) && !bitBucketList.length && 'No results found') ||
